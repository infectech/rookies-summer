 ...
 * The client sends a requestId (a UUID generated once per checkout
 * attempt, reused across its own retries) with every order. doPost()
 * remembers only the MOST RECENT requestId -> orderId pair and, if the
 * same requestId comes in again — e.g. the client retried after a
...
// Remembers only the SINGLE most recent requestId -> orderId pair, to
// short-circuit a client's own immediate retry. Two plain properties
// (not a JSON blob) keeps each check to one cheap property read.
const LAST_REQUEST_ID_KEY = "lastOrderRequestId";
const LAST_REQUEST_ORDER_ID_KEY = "lastOrderRequestOrderId";
